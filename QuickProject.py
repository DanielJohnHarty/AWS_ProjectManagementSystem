...
def destroy_instance():
    ...
    try:
        ...
        #exit
    try:
        ec2.release_address(AllocationId=data_dict['allocation_id'])
        # ec2_release_address_waiter = ec2.get_waiter('instance_terminated')
        # ec2_waiter.wait(InstanceIds=[data_dict['ec2_instance']])
...
def build_vpc(ec2_client):
    ...
    try:
        ...
        ec2_client.create_tags(Resources=[VpcId], Tags=[{'Key':'Name', 'Value':instance_identifier+'_VPC'}])
...
def build_igw(ec2_client, VpcId):
    ...
    try:
        ...
        ec2_client.create_tags(Resources=[igw_id], Tags=[{'Key':'Name', 'Value':instance_identifier+'_IGW'}])
...
def build_subnet(ec2_client, VpcId):
    data_dict = load_data_from_pickle()
    try:
        subnet_data = ec2_client.create_subnet(VpcId=VpcId,
                                CidrBlock=PUBLIC_SUBNET_CIDR_BLOCK,
                                )
        subnet_id = subnet_data['Subnet']['SubnetId']
        ec2_client.create_tags(Resources=[subnet_id], Tags=[{'Key':'Name', 'Value':instance_identifier+'_Public_Subnet'}])
...
def build_security_group(ec2_client, VpcId):
    ...
    try:
        security_group_response_data = \
                ec2_client.create_security_group(GroupName=instance_identifier+'_SECURITY_GROUP',
        ...
        ec2_client.create_tags(Resources=[security_group_id], Tags=[{'Key':'Name', 'Value':instance_identifier+'_SECURITY_GROUP'}])
    except Exception as e:
        print(e)
...
def build_ec2_instance(ec2_client,security_group_id, subnet_id, key_pair_name):
    try:
        instance_data = ec2_client.run_instances(ImageId=instance_ami,
        ...
        ec2_client.create_tags(Resources=[instance_id], Tags=[{'Key':'Name', 'Value':instance_identifier+'_Instance'}])
...
def log_initialization_to_console(ec2_client, public_ip, instance_access_port, instance_identifier):
    
    while True:
        current_status_details = ec2_client.describe_instance_status(InstanceIds =[instance_identifier])
        current_status_name = current_status_details['InstanceStatuses'][0]['InstanceState']['Name']
